...
int main(int argc,char* argv[])
{
  ...
  printf("ctime:%x - %s",(int)buf.st_ctime,ctime(&buf.st_ctime));  //마지막 수정 시각
  printf("atime:%x - %s",(int)buf.st_atime,ctime(&buf.st_atime));  //마지막 엑세스 시각
  printf("mtime:%x - %s",(int)buf.st_mtime,ctime(&buf.st_mtime));  // 마지막 변경 시각
